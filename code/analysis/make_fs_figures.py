...
def fig_c_trajectory(matrix, backbones, suite, out_dir, rendered, name, title):
    ...
    fig, axes = plt.subplots(1, len(bbs), figsize=(FS.FULL_W if len(bbs) > 1 else FS.COL_W, 2.55),
...
def fig_fs_seed_consistency(matrix, backbones, out_dir, rendered):
    fig, ax = plt.subplots(figsize=(FS.COL_W, 2.7))
...
def fig_fs_dprime_falsifier(matrix, backbones, out_dir, rendered, suite="ucit"):
    n = len(backbones)
    fig, axes = plt.subplots(1, n, figsize=(FS.FULL_W, 2.7), sharey=True, squeeze=False)
    axes = axes[0]
...
def fig_fs_lamF_sweep(matrix, backbones, out_dir, rendered, suite="ucit"):
    fig, axes = plt.subplots(1, 2, figsize=(FS.FULL_W, 2.6), squeeze=False)
    ax1, ax2 = axes[0]
    drew = False
    for bb in backbones:
...
def fig_fs_scorecard(matrix, backbones, out_dir, rendered):
    ...
    n = max(1, len(panels))
    fig, axes = plt.subplots(1, n, figsize=(min(FS.FULL_W, 2.2 * n + 1.0), 2.7), squeeze=False)
    axes = axes[0]
    if not panels:
