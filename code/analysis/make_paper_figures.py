...
def build_fig1(mcj, adv_f1, out_dir, rendered):
    ...
    # ---- layout: two row-blocks (paradox / resolution), each 1x2 -----------
    fig = plt.figure(figsize=(FS.FULL_W, FS.h_full(4.85)))
    gs_top = fig.add_gridspec(1, 2, left=0.085, right=0.975, top=0.845,
                              bottom=0.590, wspace=0.28)
    gs_bot = fig.add_gridspec(1, 2, left=0.085, right=0.975, top=0.380,
                              bottom=0.075, wspace=0.28)
    ...
    ax_f1.text(4.14, 0.5 * (seq_f1[0] + seq_f1[4]), "$-$%.03f" % f1_drop,
               rotation=90, ha="center", va="center", fontsize=6.6,
               color=WORSE_FG, fontweight="bold")
    ax_f1.text(0.03, seq_f1[0] + 0.004, "base $S_0$", fontsize=5.9,
               color="#6b6b6b", va="bottom")
    ax_f1.set_ylabel("POPE-adv. F1")
    fpad = 0.10 * (max(seq_f1) - min(seq_f1))
    ax_f1.set_ylim(min(seq_f1) - fpad - 0.006, max(seq_f1) + fpad)
    ax_f1.set_title("(a) Discriminative metric", fontsize=7.4)
    ...
    ax_ch.text(4.14, 0.5 * (seq_chair[0] + seq_chair[4]),
               "$-$%.03f" % chair_drop, rotation=90, ha="center", va="center",
               fontsize=6.6, color=BETTER_FG, fontweight="bold")
    ax_ch.text(0.03, seq_chair[0] + 0.0012, "base $S_0$", fontsize=5.9,
               color="#6b6b6b", va="bottom")
    ax_ch.set_ylabel("raw CHAIR$_i$")
    cpad = 0.12 * (max(seq_chair) - min(seq_chair))
    ax_ch.set_ylim(min(seq_chair) - cpad, max(seq_chair) + cpad)
    ax_ch.set_title("(b) Generative metric", fontsize=7.4)
    ...
    # Label the swing horizontally in the empty upper band (clear of the line).
    # Same two statistics as panel (d): within-arm range, endpoint displacement.
    ax_c.text(1.55, c_hi_i + 0.185,
              "range %.2f    endpoint %+.2f" % (c_swing, c_move),
              ha="center", va="center", fontsize=6.7,
              color=FS.SEQ_COLOR, fontweight="bold")
    ax_c.annotate("$S_4$: conservative\nswing", xy=(4.0, seq_c[4]),
                  xytext=(3.0, seq_c[4] - 0.10), ha="center", va="top",
                  fontsize=6.3, color="#2a2a2a",
                  arrowprops=dict(arrowstyle="-|>", color="#555555", lw=0.8))
    ax_c.set_ylabel("criterion $c$")
    cpad2 = 0.16 * c_swing
    ax_c.set_ylim(c_lo_i - cpad2 - 0.02, c_hi_i + cpad2 + 0.14)
    ax_c.set_xlabel("training stage (task)")
    ax_c.set_title("(c) SDT: the answer criterion $c$ moves", fontsize=7.4)
    ...
    ax_d.text(2.0, d_mid + 0.30 * c_axspan,
              "range %.2f    endpoint %+.2f" % (d_range, d_move),
              ha="center", va="center", fontsize=6.7, color=FS.SEQ_COLOR,
              fontweight="bold")
    ax_d.set_ylabel("$d'$")
    ax_d.set_xlabel("training stage (task)")
    ax_d.set_title("(d) SDT: discrimination $d'$ moves %.0f$\\times$ less"
                   % range_ratio, fontsize=7.4)

    # ---------------- row banners + verdict tags ----------------------------
    fig.text(0.5, 0.972,
             "Same checkpoints, opposite verdicts",
             ha="center", va="center", fontsize=9.2, fontweight="bold")
    fig.text(0.5, 0.935,
             "two field-standard hallucination metrics disagree on the "
             "identical LLaVA-1.5-7B checkpoints",
             ha="center", va="center", fontsize=6.7, color="#444444")
    # Verdict tags sit in the clear band just under the top row (col centres
    # ~0.30 / ~0.76), well above the resolution banner below them.
    fig.text(0.30, 0.520, "F1 falls $\\Rightarrow$ reads as MORE hallucination",
             ha="center", va="center", fontsize=6.9, color=WORSE_FG,
             fontweight="bold",
             bbox=dict(boxstyle="round,pad=0.30", fc=WORSE_BG, ec=WORSE_FG,
                       lw=0.6))
    fig.text(0.76, 0.520, "CHAIR falls $\\Rightarrow$ reads as LESS hallucination",
             ha="center", va="center", fontsize=6.9, color=BETTER_FG,
             fontweight="bold",
             bbox=dict(boxstyle="round,pad=0.30", fc=BETTER_BG, ec=BETTER_FG,
                       lw=0.6))
    fig.text(0.5, 0.442,
             "The resolution: a signal-detection split of the same POPE "
             "evaluations",
